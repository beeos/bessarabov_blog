...
эксперементальных проектов — у меня куплено несколько доменов и https
сертификатов. Мне хочется иметь одну точку, из которой я могу узнать
...
статусом 200, достаточно ли у меня денег на мобильном телефоне — вообщем,
...
хочется получать информацию мнгновенно, а запуск и прогон всех тестов шел
...
Сумирую. Мне хотелось получить систему, которая хранит информацию об
...
такой системы вебинтерфес избыточен — вполне достаточно получать данные
только в виде JSON.
...
Сейчас я покажу как равернуть работающую систему и начать ей пользоватся.
...
Давайте отправим в curry информацю об объекте:
...
Сейчас в curry у нас теперь есть информаци о двух объектах. Кроме ручки set
...
Это ручка возращает точно такую же структуру, что и get, но и с объектами в
статусе ok:
...
Когда-то давно в детстве в каком-то журнале (кажется, это был Юнный Техник),
...
мнгновенно. И благодаря созданию и внедрению curry я еще немного продвинулся
в этом направлении.
...
В качесте примера: у меня ежедневно запускаются проверка что все мои домены
...
Проект curry — это эксперимент. Для меня эту штука феерчино удобна и мне
